...
위의 설명과 같이 통일된 규약이 없다면 코드의 의도나 오류를 찾기 어렵기 때문에 꼭 필요한 단계라 정의되어 있습니다. 특히 여러명의 사람들과 진행했기에 꼭 필요한 단계였죠. 
...
1. 함수명 폴더 구조
    ...
    이 당시에는 복잡한 폴더 구조를 가질 경우에만 *함수명 폴더로 분리*해 놓는 것*을 권장했어요. 대부분의 폴더 구조가 *components 폴더 내에 함수명 폴더*로 이루어졌기에 권장했던 방식이었죠.
